...
const LPCompositionRow = (props: { row: Notification }) => {
  ...
  const poolName = (pool: LiquidityPoolHistory): string => {
    return pool.name
      ? pool.name
      : `${dexLpNames[pool.dex]} ${pool.balances[0].coins
  ...
  return (
    <React.Fragment>
      <TableRow sx={{ "& > *": { borderBottom: "unset" } }}>
        ...
        <TableCell>{`${row.data.token} weight ${
          weightChange(row.data) < 0 ? "decreased" : "increased"
        } to ${percentageFormat(currentWeight(row.data))}`}</TableCell>
        ...
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ margin: 1 }}>
              ...
              <br />
              <Typography variant="body2">
                {`The weight of ${row.data.token} ${
                  weightChange(row.data) < 0 ? "decreased" : "increased"
                } by ${percentageFormat(weightChange(row.data))}.`}
              </Typography>
              <Typography variant="body2">
                {`The current balance of the ${
                  row.data.token
                } in a pool is ${numberFormat(row.data?.balance, 2)},
                                    which is a ${percentageFormat(
                                      currentWeight(row.data)
                                    )} of the pool.`}
              </Typography>
            </Box>
          </Collapse>
